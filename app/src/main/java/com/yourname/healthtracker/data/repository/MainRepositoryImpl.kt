...
//    // 🍽️ Готовые блюда и фаст-фуд
    ...
    override suspend fun getOrCreateDay(date: String): FoodDay {
        val existingDay = daysDao.getItemByDate(date)

        return if(existingDay!=null) {
            existingDay
        } else {
            val newDay = FoodDay(date = date)
            daysDao.insertItem(newDay)
            newDay
        }
    }

    override suspend fun getOrCreateProfile(): UserProfile {
        val existingProfile = profileDao.getUserProfile()
